/**
 * [DEPRECATED] Advanced TypeScript Error Finder
 * 
 * NOTE: This utility is deprecated. Please use the enhanced TypeScript Error Management System
 * (enhanced-ts-error-scanner.ts) which provides more comprehensive error analysis,
 * cascade detection, and improved security.
 * 
...
export async function findTypeScriptErrors(
  ...
  const validSortBy: 'severity' | 'file' | 'category' | 'code' = 
    (opts.sortBy === 'severity' || opts.sortBy === 'file' || opts.sortBy === 'category' || opts.sortBy === 'code')
    ? opts.sortBy
    : 'severity';
    
  sortErrors(result.errors, validSortBy);
...
function categorizeError(code: number, message: string, file?: string): ErrorCategory {
  ...
  if (file && file.endsWith('.tsx')) {
    ...
    if (message.includes('props') && message.includes('is not assignable to')) {
      return 'type_mismatch';
    }
    
    if (message.includes('children') && message.includes('type')) {
      return 'type_mismatch';
    }
...
function getSuggestedFix(code: string, message: string, category: ErrorCategory): string | undefined {
  ...
  // React component specific suggestions
  if (code.includes('TS') && message.toLowerCase().includes('react')) {
    if (message.includes('props')) {
      return 'Define a proper interface for component props';
    }
    
    if (message.includes('children')) {
      return 'Define children prop in component props interface';
    }
    
    if (message.includes('event') || message.includes('handler')) {
      return 'Use correct event handler type from React.FormEvent, React.MouseEvent, etc.';
    }
  }
  ...
  switch (category) {
    ...
    case 'property_access':
      return 'Ensure the property exists on the object type';
    case 'interface_mismatch':
      return 'Make sure all required interface properties are implemented';
    case 'generic_constraint':
      return 'Verify the type meets all constraints of the generic parameter';
    case 'declaration_error':
      return 'Fix duplicate declarations or naming conflicts';
    case 'type_mismatch':
      return 'Ensure types are compatible or add proper type conversion';
    default:
      return undefined;
  }
}
...
async function outputResults(
  ...
  const validFormat: 'json' | 'markdown' | 'console' = 
    (options.outputFormat === 'json' || options.outputFormat === 'markdown' || options.outputFormat === 'console')
